...
export class DBToFrontend {
  ...
  public static convertTimesheets(
    timesheets: dbTypes.TimeSheetSchema[]
  ): frontendTimesheetTypes.FrontendTimeSheetSchema[] {
  ...
  public static toFrontendTimesheet(
    timesheet: dbTypes.TimeSheetSchema
  ): frontendTimesheetTypes.FrontendTimeSheetSchema {
  ...
  private static toFrontendStatus(
    status: dbTypes.TimesheetStatus
  ): frontendTimesheetTypes.StatusType {
  ...
  private static toFrontendRows(
    rows: dbTypes.TimesheetEntrySchema[]
  ): frontendRowTypes.RowSchema[] {
    if (rows === undefined) {
      return [];
    }
    return rows.map((row) => {
  ...
  private static toFrontendScheduleData(
    rows: dbTypes.ScheduleEntrySchema[]
  ): frontendRowTypes.ScheduledRowSchema[] {
    if (rows === undefined) {
      return [];
    }
    return rows.map((row) => {
  ...
  private static toFrontendRowEntry(
    row: dbTypes.TimeEntrySchema
  ): frontendRowTypes.TimeRowEntry {
    if (row === undefined) {
      return undefined;
    }
  ...
  private static toFrontendComments(
    comments: dbTypes.NoteSchema[]
