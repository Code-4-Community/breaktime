...
export class ItemsDelegator {
  ...
  public AttributeToModify(
    body: InsertRequest | DeleteRequest | UpdateRequest
  ) {
    switch (body.Type) {
...
export class HoursDataOperations implements ItemsOperations {
  public Insert(timesheet: TimeSheetSchema, body: InsertRequest) {
    ...
    if (idx === 0) {
      ...
      return {
        ...timesheet,
        HoursData: [...data.slice(0, idx), item, ...data.slice(idx + 1)],
      };
    }
  }
  ...
  public Update(timesheet: TimeSheetSchema, body: UpdateRequest) {
    if (
      timesheet.HoursData?.filter((row) => row.EntryID === body.Id).length === 0
    ) {
  ...
  public StatusChange(timesheet: TimeSheetSchema, body: StatusChangeRequest) {
    if (timesheet.TimesheetID !== body.TimesheetId) {
      throw new Error(
        "Requested timesheet does not match timesheet ID of timesheet being updated"
      );
    }
    ...
    console.log(
      "Handling Status Change Request for timesheet %s",
      body.TimesheetId.valueOf()
    );
...
export class ScheduledDataOperations implements ItemsOperations {
  public Insert(timesheet: TimeSheetSchema, body: InsertRequest) {
    ...
    if (idx === 0) {
      ...
      return {
        ...timesheet,
        ScheduleData: [...data.slice(0, idx), item, ...data.slice(idx + 1)],
      };
    }
  }
  public Delete(timesheet: TimeSheetSchema, body: DeleteRequest) {
    return {
      ...timesheet,
      ScheduleData: timesheet.ScheduleData.filter(
        (row) => row.EntryID !== body.Id
      ),
    };
  }
...
export class NotesOperations implements ItemsOperations {
  public Insert(timesheet: TimeSheetSchema, body: InsertRequest) {
    return {
      ...timesheet,
      WeekNotes: [...timesheet.WeekNotes, NoteSchema.parse(body.Item)],
    };
  }
  public Delete(timesheet: TimeSheetSchema, body: DeleteRequest) {
    return {
