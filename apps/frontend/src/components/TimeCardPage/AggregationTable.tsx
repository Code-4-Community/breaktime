...
function AggregationTable(props: AggregationProps) {
  ...
  props.timesheets.forEach((sheet) => {
    if (sheet.TableData !== undefined) {
      sheet.TableData.forEach((entry) => {
        if (
          entry.AssociateTimeEntry !== undefined &&
          entry.AssociateTimeEntry.StartDateTime !== undefined &&
          entry.AssociateTimeEntry.EndDateTime !== undefined
        ) {
          totalHoursForEachDay[moment.unix(entry.Date).format("MM/DD/YY")] +=
            Number(
              entry.AssociateTimeEntry.EndDateTime -
                entry.AssociateTimeEntry.StartDateTime
            );
        }
  ...
  return (
    <Table>
      ...
      <Tbody>
        {aggregatedRows.map((totalRow) => {
          return (
            <Tr key={uuidv4()}>
              <Td>{totalRow.Date}</Td>
              <Td>{(totalRow.Duration / 60).toFixed(2)}</Td>
            </Tr>
          );
        })}
        <Tr>
          <Td>Total Hours</Td>
          <Td>{(totalHours / 60).toFixed(2)}</Td>
        </Tr>
      </Tbody>
    </Table>
  );
